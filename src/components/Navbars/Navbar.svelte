<script>
  let navbarTitles = [
      { name: 'Dashboard', link: '#/' },
      { name: 'Sipariş Kargo Durumları', link: '#/productCargoStatus' },
      ...
      <div class="flex h-16 items-center justify-between">
        <div class="flex items-center">
          <div class="flex-shrink-0">
            <img class="h-8" src="src/assets/img/logo.png" alt="ModaSelvim">
          </div>
          <div class="hidden md:block">
            <div class="ml-10 flex items-baseline space-x-4">
              <!-- Current: "bg-gray-900 text-white", Default: "text-gray-300 hover:bg-gray-700 hover:text-white" -->
              {#each navbarTitles as title}
                <a href={title.link} class="text-gray-300 hover:bg-gray-700 hover:text-white rounded-md px-3 py-2 text-sm font-medium">{title.name}</a>
              {/each}
            </div>
          </div>
        </div>
